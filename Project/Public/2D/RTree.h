...
#define RASSERT assert // RTree uses RASSERT( condition )
#ifndef Min
  #define Min std::min
#endif //Min
#ifndef Max
...
protected:
  ...
  tlib::TDynPool<Node, 16>  _nodePool;
  tlib::TDynPool<ListNode, 8>      _listNodePool;
#endif
};
...
void RTREE_QUAL::Insert(const ELEMTYPE a_min[NUMDIMS], const ELEMTYPE a_max[NUMDIMS], const DATATYPE& a_dataId)
{
  ...
  for(int index=0; index<NUMDIMS; ++index)
  {
    RASSERT(a_min[index] <= a_max[index]);
  }
#endif //_DEBUG
...
#else // RTREE_DONT_USE_MEMPOOLS
  // EXAMPLE
  newNode = CREAT_FROM_POOL(_nodePool);
#endif // RTREE_DONT_USE_MEMPOOLS
  InitNode(newNode);
  return newNode;
}
...
#else // RTREE_DONT_USE_MEMPOOLS
  // EXAMPLE
    return CREAT_FROM_POOL(_listNodePool);
#endif // RTREE_DONT_USE_MEMPOOLS
}
...
#else // RTREE_DONT_USE_MEMPOOLS
  // EXAMPLE
  _listNodePool.Recover(a_listNode);
#endif // RTREE_DONT_USE_MEMPOOLS
}
...
{
  RASSERT(a_node && a_newNode);
  RASSERT(a_level >= 0 && a_level <= a_node->m_level);
...
{
  RASSERT(a_root);
  RASSERT(a_level >= 0 && a_level <= (*a_root)->m_level);
#ifdef _DEBUG
  for(int index=0; index < NUMDIMS; ++index)
  {
    RASSERT(a_branch.m_rect.m_min[index] <= a_branch.m_rect.m_max[index]);
  }
#endif //_DEBUG  
...
{
  RASSERT(a_node && (a_index >= 0) && (a_index < MAXNODES));
  RASSERT(a_node->m_count > 0);
...
void RTREE_QUAL::SplitNode(Node* a_node, const Branch* a_branch, Node** a_newNode)
{
  ...
  *a_newNode = AllocNode();
  ...
  RASSERT((a_node->m_count + (*a_newNode)->m_count) == parVars->m_total);
}
...
void RTREE_QUAL::GetBranches(Node* a_node, const Branch* a_branch, PartitionVars* a_parVars)
{
  ...
  RASSERT(a_node->m_count == MAXNODES);
...
void RTREE_QUAL::ChoosePartition(PartitionVars* a_parVars, int a_minFill)
{
  ...
  RASSERT((a_parVars->m_count[0] + a_parVars->m_count[1]) == a_parVars->m_total);
  //RASSERT((a_parVars->m_count[0] >= a_parVars->m_minFill) && 
...
void RTREE_QUAL::LoadNodes(Node* a_nodeA, Node* a_nodeB, PartitionVars* a_parVars)
{
  ...
  for(int index=0; index < a_parVars->m_total; ++index)
  {
    RASSERT(a_parVars->m_partition[index] == 0 || a_parVars->m_partition[index] == 1);
...
{
  RASSERT(a_parVars);
  RASSERT(PartitionVars::NOT_TAKEN == a_parVars->m_partition[a_index]);
...
{
  RASSERT(a_rect && a_node && a_listNode);
  RASSERT(a_node->m_level >= 0);
